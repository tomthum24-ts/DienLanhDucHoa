...
namespace Model.Dao
{
    public class CategoryDao
    {
        ...
        {
            return db.Categories.OrderBy(x => x.Stt).ToPagedList(page, pageSize);
        }
        public List<Category> ListAll()
        {
            return db.Categories.Where(x => x.Status == true).OrderBy(x=>x.Stt).ToList();
        }
        public List<Category> GetCategory(long id)
        {
            return db.Categories.Where(x => x.Status == true && x.ID == id).OrderBy(x => x.Stt).ToList();
        }
        public Category GetByIdCategory(long id)
        {
            return db.Categories.Where(x => x.Status == true && x.ID==id).OrderBy(x => x.Stt).FirstOrDefault();
        }
